...
function ordenarDeMayorAMenor(listaDeNumeros) {
    ...
    return listaDeNumeros.sort().reverse()
}
...
function encontrarNumeroMayor(listaDeNumeros) {
    ...
    for(x=0; x < listaDeNumeros.length; x++){
        if(listaDeNumeros[x] > maxN){
            maxN = listaDeNumeros[x]
        }

    }
    return maxN
 // en caso de que no funcione podemos utilizar, utlizar este-> return Math.max(...listaDeNumeros);
}
...
function enumerarLista(listaDePalabras) {
    ...
    let palabrasEnumeradas = listaDePalabras.join(", ");

    
    let oracionCompleta = `${palabrasEnumeradas} y ${ultimaPalabra}.`;

    
    oracionCompleta = oracionCompleta.charAt(0).toUpperCase() + oracionCompleta.slice(1);
